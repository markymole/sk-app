...
$notificationed = new Notifications();
$image = new Images();
$date = new General();
$comments = new Comments();

$notifications = $notificationed->getUserNotification($userId);
...
if ($notifications !== false) {
    if (!empty($notifications)) {
        foreach ($notifications as $notification) {
            ...
            if ($seen) {
                $bg_class = "flex px-4 py-3 bg-white hover:bg-gray-200 dark:hover:bg-gray-700";
            } else {
                $bg_class = "flex px-4 py-3 bg-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700";
            }


            $notif_fro_img = $image->getUserProfileImage($notif_fro_id, $notif_from_gender);
            $notif_formatted_date = $date->formatDate($notif_created_at);
            ...
            if ($context === "Post") {
                ...
                <path d="M14 6c-.762 0-1.52.02-2.271.062C10.157 6.148 9 7.472 9 8.998v-2.24c0-1.519 1.147 2.839 2.71 2.935.214.013.428.024.642.034.2.009.385.09.518.224l-2.35 2.35a.75.75 0 001.28-.531v-2.07c1.453-.195 2.5-1.463 2.5-2.915V8.998c0-1.526-1.157-2.85-2.729-2.936A41.645 41.645 0 0014 6z" />
            </svg>';
            ...
            <a href="$redirect" class="$bg_class" data-notification-id="$notification_id" id="notification-item">
                <div class="flex-shrink-0">
                    <img class="rounded-full w-11 h-11 object-cover" src="$notif_fro_img" alt="User image">
                ...
                <div class="w-full pl-3">
                    ...
                    <div class="text-xs text-blue-600 dark:text-blue-500">$notif_formatted_date</div>
                </div>
            </a>
HTML;
        }
